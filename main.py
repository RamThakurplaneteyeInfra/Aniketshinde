...
def calculate_npk_for_plot1(geometry, plantation_date=PLANTATION_DATE):
    ...
    try:
        ...
        # ? Vegetation cover–based NPK analysis
...
def analyze_plot(
    ...
    """Analyze soil parameters for a specific plot including Fe Index (using Sentinel-1 SAR and current date)"""
    ...
    stats = calculate_mean_statistics(geometry, fe_days_back=fe_days_back)
    area = geometry.area().divide(10000).getInfo()
...
@app.post("/required-n/{plot_name}", response_model=RequiredNResponse1)
def get_required_n_per_acre(
    plot_name: str,
    plantation_date: Optional[str] = Query(None, description="(Optional) Plantation date in YYYY-MM-DD"),
    end_date: str = Query(default=datetime.now().strftime('%Y-%m-%d'), description="End date for yield analysis")
):
    """Calculate required nitrogen, phosphorus, and potassium per acre for a specific plot."""
    ...
    if not geometry:
        raise HTTPException(status_code=400, detail=f"Invalid geometry for plot '{plot_name}': geometry is None")
    ...
    try:
        ...
        if geom_type not in ["Polygon", "MultiPolygon"]:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid geometry type for plot '{plot_name}': {geom_type}. Area calculation requires Polygon or MultiPolygon."
            )
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Failed to validate geometry for plot '{plot_name}': {str(e)}")
 
    # --- Calculate area in hectares ---
    area_hectares = None
    try:
        area_hectares = calculate_area_hectares(geometry)
    except Exception as e:
        print(f"EE area calculation failed: {e}")
    ...
    if area_hectares is None:
        try:
            ...
            if isinstance(geometry, dict) and "coordinates" in geometry:
                shapely_geom = shape(geometry)
                area_hectares = shapely_geom.area / 10000.0
        except Exception as e:
            print(f"Fallback area calculation failed for '{plot_name}': {e}")
 
    if not area_hectares or area_hectares <= 0:
        raise HTTPException(
            status_code=400,
            detail=f"Failed to calculate area for plot '{plot_name}'. Ensure valid GeoJSON or EE geometry is provided."
        )
 
    area_acres = round(area_hectares * 2.47105, 2)
 
    # --- Handle plantation date ---
    if not plantation_date:
        plantation_date = properties.get("plantation_date")
        if not plantation_date:
            raise HTTPException(
                status_code=400,
                detail=f"No plantation date found for plot '{plot_name}'. Please provide one manually."
            )
    ...
    days_since_plantation = (end_dt - plantation_dt).days
    if days_since_plantation < 1:
        raise HTTPException(status_code=400, detail="Days since plantation must be >= 1")
 
    # --- Get NPK and vegetation proxy ---
    npk_result = calculate_npk_for_plot1(geometry, plantation_date)
    if "error" in npk_result:
        raise HTTPException(status_code=500, detail=npk_result["error"])
 
    gndvi_value = npk_result.get("average_n", 0) or 0  # vegetation nitrogen proxy
 
    # --- Determine baseline soil analysis and max yield ---
    if 0 <= days_since_plantation <= 60:
        soil_analysis_value, max_yield = 160, 20
    elif 61 <= days_since_plantation <= 90:
        soil_analysis_value, max_yield = 240, 40
    elif 91 <= days_since_plantation <= 120:
        soil_analysis_value, max_yield = 300, 60
    else:
        soil_analysis_value = 430
        # Mature crops: use BRIX yield scaled by area
        start_date = (end_dt - timedelta(days=18)).strftime("%Y-%m-%d")
        brix_stats = calculate_brix_sugar_stats1(start_date, end_dt)
        if plot_name not in brix_stats:
            raise HTTPException(status_code=500, detail="No yield data found for this plot.")
        if area_acres < 1:
            max_yield = brix_stats[plot_name]["sugar_yield"]["mean"]/2.471
        else:
            max_yield = brix_stats[plot_name]["sugar_yield"]["mean"]/area_acres
 
    # --- Calculate required NPK ---
    required_n = max(0, (max_yield * 6 - gndvi_value))
    ...
    if soilN <= 0:
        soilN = 120
        required_n = 0  # enforce 0 if soilN falls back
        required_p = required_n * 0.33
        required_k = required_n * 0.67
    ...
    return {
        ...
        "soil_analysis_value": soil_analysis_value,
        "max_yield": round(max_yield, 2),
        "required_n_per_acre": required_n,
        "gndvi": round(gndvi_value, 2),
        "soilN": round(soilN, 2),
        "soilP": round(soilP, 2),
        "soilK": round(soilK, 2),
...
async def refresh_plots():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to refresh plots: {str(e)}")
