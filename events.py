...
def calculate_all_stats_soil(scale=250):
    ...
    # 🔹 Build batch computation for ALL plots
    batch_computations = {}
    ...
    # 🔹 Single getInfo() call for ALL plots
    try:
...
def get_optimal_harvest_window(plantation_date_str: str, variety_type: str = "Mid") -> Dict[str, Any]:
    ...
    # Optimal harvest window is ±15 days around the target harvest date
...
def get_biomass_summary_batch(plot_dict) -> Dict[str, BiomassStats]:
    ...
    # 🔹 Single Sentinel-1 collection for all plots (reused)
    sentinel1 = (ee.ImageCollection('COPERNICUS/S1_GRD')
                 .filterDate(start_date, end_date)
                 .filter(ee.Filter.listContains('transmitterReceiverPolarisation', 'VV'))
                 .filter(ee.Filter.listContains('transmitterReceiverPolarisation', 'VH'))
    ...
    # 🔹 Build computations for all plots
    all_computations = {}
    ...
    # 🔹 Convert to BiomassStats objects
    biomass_stats = {}
    for plot_name in plot_dict.keys():
...
def calculate_brix_sugar_stats(start_date, end_date):
    ...
    for feature in reduced.get('features', []):
        ...
        stats[plot_name] = {
            ...
            "sugar_yield": {
                "mean": safe_round(props, "SugarYield_mean")/2.471,
                "min": safe_round(props, "SugarYield_min")/2.471,
                "max": safe_round(props, "SugarYield_max")/2.471,
                "median": safe_round(props, "SugarYield_median")/2.471,
                "stdDev": safe_round(props, "SugarYield_stdDev")/2.471,
...
def get_all_stats(
    ...
    brix_stats = calculate_brix_sugar_stats(start_date, end_date)
    
    # 🔹 Batch area calculations
    area_computations = {}
    for plot_name, plot in plot_dict.items():
        roi = plot["geometry"]
        area_computations[plot_name] = roi.area().divide(10000)
    ...
    # 🔹 Batch biomass calculations - ONE API CALL FOR ALL PLOTS
    biomass_results = get_biomass_summary_batch(plot_dict)
    
    # 🔹 Setup factory point for distance calculations (local, no API calls)
    factory_point = None
    factory_coords = None
    ...
    for plot_name, plot in plot_dict.items():
        ...
        # Get batched area
        area_ha = batch_areas.get(plot_name, 0)
        if area_ha:
        ...
        # 🔹 Calculate distance (local computation, very fast)
        distance_km = None
        ...
        # 🔹 Calculate harvest info from plot properties (local calculation, very fast)
...
async def refresh_from_django():
    ...
    try:
        global plot_dict
        print("🔄 Manual refresh from Django requested for events.py...")
...
if __name__ == "__main__":
    uvicorn.run("events:app", host="0.0.0.0", port=9000, reload=True)
